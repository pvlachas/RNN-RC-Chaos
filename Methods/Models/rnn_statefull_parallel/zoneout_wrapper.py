...
class ZoneoutWrapper(tf.contrib.rnn.RNNCell):
    ...
    def __init__(self, cell, zoneout_drop_prob, is_training, dtype=tf.float64):
        self._cell = cell
    ...
    def zoneoutLoop(self, state, new_state):
        ...
        for i, state_element in enumerate(state):
            ...
            random_tensor += tf.random_uniform(tf.shape(state_element), dtype=self._dtype)
            # 0. if [zoneout_prob, 1.0) and 1. if [1.0, 1.0 + zoneout_prob)
            binary_tensor = tf.floor(random_tensor)
            final_new_state[
            i] = (new_state[i] - state_element) * binary_tensor + state_element
        return final_new_state

    def normalLoop(self, state, new_state):
        final_new_state = [new_state[0], new_state[1]]
        for i, state_element in enumerate(state):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        final_new_state = tf.cond(self._is_training, lambda: self.zoneoutLoop(state, new_state), lambda: self.normalLoop(state, new_state))

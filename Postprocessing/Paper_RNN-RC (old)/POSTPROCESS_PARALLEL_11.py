...
if __name__ == "__main__":
    ...
    if system_name == "KuramotoSivashinskyGP512":
        ...
        lambda1=0.094    
        RDIM_VALUES = [512]
        xlims = np.array([600, 1000])*dt*lambda1
        ylims = [[-8, 2]]
        ...
        for r in range(len(RDIM_VALUES)):
            ...
            for xlim in xlims:
                for ylim in ylims:
                    ...
                    fig.savefig(fig_path + '/P_LOG_RMNSE_BBO_{:}_2_M_RDIM_{:}_{:}.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM, bounds_iter), bbox_inches='tight')
                    plt.close()
            ...
            for xlim in xlims:
                for ylim in ylims:
                    ...
                    NUM_ACC_PRED_STR_SHORT = "NAP" if NUM_ACC_PRED_STR =="num_accurate_pred_050_avg_TEST" else 0
                    fig.savefig(fig_path + '/P_LOG_RMNSE_BBO_{:}_2_M_RDIM_{:}_{:}_LEGEND.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM, bounds_iter), bbox_extra_artists=(lgd,), bbox_inches='tight')
                    plt.close()
                    bounds_iter = bounds_iter + 1

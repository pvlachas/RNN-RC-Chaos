...
# python3 POSTPROCESS_5.py --system_name Lorenz96_F8GP40R40 --Experiment_Name Experiment_Daint_Large
...
for NUM_ACC_PRED_STR in ["num_accurate_pred_050_avg_TEST"]:
    ...
    # for RDIM in [RDIM_VALUES[0]]:
        ...
        for MODEL in range(N):
            ...
            for model_name_key in model_test_dict:
                ...
                if REGEX.match(model_name_key):
                    ...
                    # history = int(np.round(1/(1-radius)))
                    history = float(radius)
        ...
        fig.savefig(fig_path + '/{:}_2_HISTORY_RDIM_ESN_{:}.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM), bbox_inches='tight')
        lgd = ax.legend(loc="upper left", bbox_to_anchor=(1,1))
        fig.savefig(fig_path + '/{:}_2_HISTORY_ESN_RDIM_{:}_LEGEND.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM), bbox_extra_artists=(lgd,), bbox_inches='tight')
        plt.close()

        ...
        for r in range(len(RDIM_VALUES)):
                ...
                for model_name_key in model_test_dict:
                    ...
                    if REGEX.match(model_name_key):
                        ...
                        if valid_time > valid_time_:
                            ...
                            train_file_ = train_results_path + model_name_key + "/data_member_{:}.pickle"
                            ...
                            GIL = temp[-1]
                            TYPE = temp[0].split("-")[2]
                            if temp[0]=='GPU-RNN-esn-PARALLEL-NG':
                                SIZE = temp[5].split("-")[0]
                            else:
                                SIZE = temp[6].split("-")[0]
            ...
            for IC in [10,20,30,40]:
                ...
                for P in range(len(model_label_list)):
                    ...
                    error = np.sqrt(pow(prediction-target, 2.0)/pow(std_list[P], 2.0))
                ...
                fig, axes = plt.subplots(nrows=2, ncols=NUM_PLOTS,figsize=(20, 14), gridspec_kw={"width_ratios":NUM_PLOTS*[1]}, sharey=True)
                ...
                # plt.show()
                # print(ark)
                fig.savefig(fig_path + '/P_CONTOUR_BBO_{:}_2_M_RDIM_{:}_IC{:d}.png'.format(NUM_ACC_PRED_STR_SHORT, RDIM, IC), bbox_inches='tight', dpi=80)
                lgd = ax.legend(loc="upper left", bbox_to_anchor=(1,1))
                fig.savefig(fig_path + '/P_CONTOUR_BBO_{:}_2_M_RDIM_{:}_IC{:d}_LEGEND.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM, IC), bbox_inches='tight')

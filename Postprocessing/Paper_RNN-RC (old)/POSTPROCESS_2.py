...
# PLOTTING THE NUM OF ACCURATE PREDICTIONS W.R.T. TRAINING TIME
...
for NUM_ACC_PRED_STR in ["num_accurate_pred_050_avg_TEST"]:
    ...
    # for RDIM in [RDIM_VALUES[0]]:
        ...
        for MODEL in range(N):
            ...
            training_time_vec = np.reshape(training_time_vec, (-1,1))
            valid_time_vec = np.reshape(valid_time_vec, (-1,1))
            data = np.concatenate((training_time_vec, valid_time_vec), axis=1)
        ...
        fig.savefig(fig_path + '/{:}_2_TRAINTIME_RDIM{:}.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM), bbox_inches='tight')
        lgd = ax.legend(loc="upper left", bbox_to_anchor=(1,1))
        fig.savefig(fig_path + '/{:}_2_TRAINTIME_RDIM{:}_LEGEND.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM), bbox_extra_artists=(lgd,), bbox_inches='tight')

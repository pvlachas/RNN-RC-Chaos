...
# PLOTTING THE VPT BAR for KuramotoSivashinskyGP512
...
if __name__ == "__main__":
    ...
    for NUM_ACC_PRED_STR in ["num_accurate_pred_050_avg_TEST"]:
        ...
        for MODEL in range(len(model_names_str)):
            ...
            valid_time_vec.append(valid_time_)
            labels_vec.append(model_label_)
            model_color_vec.append(model_color_)
            model_hatch_vec.append(model_hatch_)
        indexes = np.arange(len(valid_time_vec))
        print(indexes)
        print(valid_time_vec)
        iter_ = 0
        for index in indexes[::-1]:
            # plt.barh(index, valid_time_vec[iter_], color=model_color_vec[iter_])
        ...
        plt.xlabel('VPT', fontsize=16)
        plt.subplots_adjust(left=0.2, right=0.9, top=0.9, bottom=0.2)
        fig.savefig(fig_path + '/P_VPT_BAR_BBO_{:}.pdf'.format(NUM_ACC_PRED_STR_SHORT), bbox_inches='tight')

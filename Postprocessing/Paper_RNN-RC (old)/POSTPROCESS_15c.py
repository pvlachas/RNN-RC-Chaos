...
# python3 POSTPROCESS_15c.py --system_name Lorenz96_F8GP40R40 --Experiment_Name="Experiment_Daint_Large"
# python3 POSTPROCESS_15c.py --system_name Lorenz96_F10GP40R40 --Experiment_Name="Experiment_Daint_Large"
...
model_names_str = [
"GPU-RNN-esn(.*)-RDIM_{:d}-N_used_(.*)-SIZE_(.*)-D_(.*)-RADIUS_(.*)-SIGMA_(.*)-DL_(.*)-NL_0-IPL_(.*)-REG_(.*)-WID_0",
"GPU-RNN-esn(.*)-RDIM_{:d}-N_used_(.*)-SIZE_(.*)-D_(.*)-RADIUS_(.*)-SIGMA_(.*)-DL_(.*)-NL_5-IPL_(.*)-REG_(.*)-WID_0",
"GPU-RNN-esn(.*)-RDIM_{:d}-N_used_(.*)-SIZE_(.*)-D_(.*)-RADIUS_(.*)-SIGMA_(.*)-DL_(.*)-NL_10-IPL_(.*)-REG_(.*)-WID_0",
"GPU-RNN-gru-RDIM_{:d}-N_used_(.*)-NUM-LAY_(.*)-SIZE-LAY_(.*)-ACT_(.*)-ISH_statefull-SL_(.*)-PL_(.*)-LR_(.*)-DKP_(.*)-ZKP_(.*)-HSPL_(.*)-IPL_(.*)-NL_0-WID_0",
"GPU-RNN-gru-RDIM_{:d}-N_used_(.*)-NUM-LAY_(.*)-SIZE-LAY_(.*)-ACT_(.*)-ISH_statefull-SL_(.*)-PL_(.*)-LR_(.*)-DKP_(.*)-ZKP_(.*)-HSPL_(.*)-IPL_(.*)-NL_2-WID_0",
]
...
for NUM_ACC_PRED_STR in ["num_accurate_pred_050_avg"]:
    ...
    for r in range(len(RDIM_VALUES)):
        ...
        fig.savefig(fig_path + '/REG_NOISE_OFSP_BBO_{:}_RDIM_{:}.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM), bbox_inches='tight')
        lgd = ax.legend(loc="upper left", bbox_to_anchor=(1,1))
        fig.savefig(fig_path + '/REG_NOISE_OFSP_BBO_{:}_RDIM_{:}_LEGEND.pdf'.format(NUM_ACC_PRED_STR_SHORT, RDIM), bbox_extra_artists=(lgd,), bbox_inches='tight')
        plt.close()

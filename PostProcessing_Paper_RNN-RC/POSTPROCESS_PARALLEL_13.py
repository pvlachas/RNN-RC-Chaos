...
# PLOTTING THE MEMORY BAR for KuramotoSivashinskyGP512
...
if __name__ == "__main__":
    ...
    for NUM_ACC_PRED_STR in ["num_accurate_pred_050_avg_TEST"]:
            ...
            for model_name_key in model_test_dict:
                ...
                if REGEX.match(model_name_key):
                    ...
                    if valid_time > valid_time_:
                        ...
                        GIL = temp[-1]
                        TYPE = temp[0].split("-")[2]
            memory = getMemoryOfParallelModel(train_path_, NPG, GS, GIL, TYPE)
            memory = float(memory)/float(int(NPG))
        ...
        indexes = np.arange(len(memory_vec))
        print(indexes)
        print(memory_vec)
        iter_ = 0
        for index in indexes[::-1]:
            plt.barh(index, memory_vec[iter_], color=model_color_vec[iter_], hatch=model_hatch_vec[iter_], ecolor="white", edgecolor="white")
            iter_ = iter_ + 1
            # plt.barh(index, valid_time_vec[iter_], color=model_color_vec[iter_], hatch=model_hatch_vec[iter_], ecolor="white", edgecolor="white")

        plt.yticks(indexes, labels_vec[::-1], fontsize=16, rotation=0)
        # ax.xaxis.major.formatter._useMathText = True
        plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
        plt.xlabel('Average RAM memory consumption [MB]', fontsize=16)
        plt.subplots_adjust(left=0.2, right=0.9, top=0.9, bottom=0.2)
        NUM_ACC_PRED_STR_SHORT = "NAP" if NUM_ACC_PRED_STR =="num_accurate_pred_050_avg_TEST" else 0
        fig.savefig(fig_path + '/P_RAM_AVERAGE_BAR_BBO_{:}.pdf'.format(NUM_ACC_PRED_STR_SHORT), bbox_inches='tight')

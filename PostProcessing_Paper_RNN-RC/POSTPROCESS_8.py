...
# python3 POSTPROCESS_8.py --system_name Lorenz3D
# python3 POSTPROCESS_8.py --system_name Lorenz96_F8GP40R40
...
# PLOTTING AVERAGE RMSE PLOT W.R.T. TIME
...
for NUM_ACC_PRED_STR in ["num_accurate_pred_050_avg_TEST"]:
    ...
    for MODEL in range(len(model_names_str)):
        ...
        for r in range(len(RDIM_VALUES)):
            ...
            model_label = model_label_str[MODEL] + " RDIM " + str(RDIM)
            model_color = model_color_str[r]
            model_marker = model_marker_str[r]
            model_markeredgewidth = model_marker_width_str[r]
            image_name = model_label_str[r]
            valid_time_ = -1
            ...
            # ax.plot(time_vector, rmnse_best, color=model_color, label=model_label, marker=model_marker, markersize=markersize, linewidth=linewidth, markeredgewidth=model_markeredgewidth)
        ...
        for xlim in xlims:
            for ylim in ylims:
                ...
                fig.savefig(fig_path + '/RMNSE_BBO_{:}_2_RDIM_{:}_{:}_{:}.pdf'.format(NUM_ACC_PRED_STR_SHORT, model_label_str[MODEL], int(1000*xlim), int(1000*ylim)), bbox_inches='tight')
                plt.close()
        ...
        for xlim in xlims:
            for ylim in ylims:
                ...
                NUM_ACC_PRED_STR_SHORT = "NAP" if NUM_ACC_PRED_STR =="num_accurate_pred_050_avg_TEST" else 0
                fig.savefig(fig_path + '/RMNSE_BBO_{:}_2_RDIM_{:}_{:}_{:}_LEGEND.pdf'.format(NUM_ACC_PRED_STR_SHORT, model_label_str[MODEL], int(1000*xlim), int(1000*ylim)), bbox_extra_artists=(lgd,), bbox_inches='tight')
                plt.close()
